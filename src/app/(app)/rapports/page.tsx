...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  const entries = movements.filter(m => m.type === MovementType.IN); const exits = movements.filter(m => m.type === MovementType.OUT);
  ...
  return <div className="space-y-6">
    ...
    <section className="grid grid-cols-2 gap-3 xl:grid-cols-4"><Metric label="Entrées" value={entries.length} detail="opérations enregistrées" icon={ArrowDownToLine} tone="green" /><Metric label="Sorties" value={exits.length} detail="opérations enregistrées" icon={ArrowUpFromLine} tone="red" /><Metric label="Stocks faibles" value={lowProducts.length} detail="à réapprovisionner" icon={AlertTriangle} tone="orange" /><Metric label="Ruptures" value={outProducts.length} detail="stock égal à zéro" icon={PackageX} tone="red" /></section>
